...
│   ├── rubricas.jsx                     # Gestión de rúbricas y criterios
│   ├── exposiciones.jsx                 # Programa de exposiciones + permisos
│   └── evaluaciones.jsx                 # Registro y consulta de evaluaciones
├── components/
│   ├── Shell.jsx                        # Layout principal con navegación
│   ├── Modal.jsx                        # Componente modal reutilizable
│   └── Icons.jsx                        # Biblioteca de iconos SVG
├── context/
│   └── AuthContext.jsx                  # Contexto de sesión JWT
├── lib/
│   └── client.js                        # Axios con interceptor Bearer token
├── utils/
│   └── helpers.js                       # colorFor, labelFor (badges de desempeño)
└── package.json
```
...
Ejecuta `datos_prueba.sql` (en `exposiciones-backend/`) en el **SQL Editor** de tu proyecto Supabase. El script comienza con un bloque `DELETE` que limpia datos anteriores — es completamente **idempotente**.
